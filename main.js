...
function send_html_server(res, data) {
  ...
  if (data.message)
    ...
    res.render('index', {base: data.from.currency, amount: data.from.amount,
                          new_base: data.to.currency, new_amount : data.to.amount, error: null
                         });
}
...
app.get('/', function (req, res) {
  res.render('index', {base: null, amount: null,
    new_base: null, new_amount : null, error: null
    });
})
